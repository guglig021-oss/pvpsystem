...
public class MatchManager {
    ...
    public void startMatchWithPersonalKits(Player p1, Player p2, ItemStack[] kit1, ItemStack[] armor1, ItemStack[] kit2, ItemStack[] armor2) {
        Arena arena = plugin.getArenaManager().getAvailableArena();
        if (arena == null) { p1.sendMessage(plugin.msg("arena-not-ready")); p2.sendMessage(plugin.msg("arena-not-ready")); return; }
        arena.setInUse(true);
    ...
    public void endMatch(Match match, UUID winnerUUID) {
        ...
        if (countdownTasks.containsKey(match)) { countdownTasks.get(match).cancel(); countdownTasks.remove(match); }
    ...
    public Match getMatch(UUID uuid) { return playerMatches.get(uuid); }
    public boolean isInMatch(UUID uuid) { return playerMatches.containsKey(uuid); }
}
